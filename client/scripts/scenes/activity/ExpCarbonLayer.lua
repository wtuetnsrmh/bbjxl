...
local ExpCarbonLayer = class("ExpCarbonLayer", function()
	return display.newLayer(MoneyRes .. "diff_bg.png")
end)

--second 服务器和cd之差
function ExpCarbonLayer:ctor(params)
	self.params = params or {}
	self.priority = self.params.priority - 4 or - 130
	self.size = self:getContentSize()
	self:anch(0.5, 0.5):pos(display.cx, display.cy)
	self.mask = DGMask:new({ item = self, priority = self.priority - 1,ObjSize = self.size,opacity = params.opacity,
		click = function()
		end,
		clickOut = function()
	...
	ui.newTTFRichLabel({text = "[color=FF533B22]推荐使用 [color=FF533B22]防御型[/color] 武将[/color]", size = 20 })
		:pos(self.size.width / 2, 45):addTo(self)

	local moneyTip={"13","30","50","75"}
	--初始化buttons：
	local xBegin = 42
	...
	for index = 1,4 do
		...
		local mBattleBtnR = DGBtn:new(MoneyRes, {"diff_bar_normal.png", "diff_bg_selected.png"},
			{	
				...
				callback = function()				
					...
					game:sendData(actionCodes.ExpBattleRequest, bin)
					game:addEventListener(actionModules[actionCodes.ExpBattleRequest], handler(self, self.canEnter))
				end,
			})
		...
		local battleInfo = expBattleCsv:getDataById(index)
		...
		if game.role.level >= battleInfo.level then
			...
			-- display.newSprite(string.format("%sexp_level_%d.png",MoneyRes,index))	
			-- 	:pos(bgSize.width / 2, bgSize.height / 2):addTo(infoBg)
		end
	end
	...
	ui.newTTFLabel({text = string.format("%d 次",tonumber(times) - game.role.expBattleCount),size = 20,color = uihelper.hex2rgb("#7ce810") })
...
--分：秒
function ExpCarbonLayer:getTimeFormateStr(seconds)
	local s = tonumber(seconds)
	return string.format("%02d:%02d", math.floor(s/60),s%60)
end

--state == 0.可以 4.等级不足 5.体力不足 6.次数 7.刷新时间
function ExpCarbonLayer:canEnter(event)

	local msg = pb.decode("SimpleEvent", event.data)
	local state = tonumber( msg.param1)
	local battleid = tonumber(msg.param2)
	if state == 0 then
		self:initExpBattleFiled(battleid)
	elseif state == 4 then
...
--创建战场：
function ExpCarbonLayer:initExpBattleFiled(battleID)

	local r = expBattleCsv:getDataById(battleID)
	switchScene("battle", { 
		battleType = BattleType.Exp, 
		carbonId = r.btres,        --战斗配表
		round = 1,                 --默认初始化第一回合
		battleindex = battleID,    --选择的管卡index
		})
end

function ExpCarbonLayer:checkGuide()
	
end

function ExpCarbonLayer:getLayer()
	return self.mask:getLayer()
end

return ExpCarbonLayer