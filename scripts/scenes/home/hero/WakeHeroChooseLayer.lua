...
local WakeHeroChooseLayer = class("WakeHeroChooseLayer", function(params)
	return display.newLayer(GlobalRes .. "inner_bg.png")
end)

function WakeHeroChooseLayer:ctor(params)
	params = params or {}
	...
	table.sort(self.heros, function(a, b) 
		...
		local factorA = a.choose * 1000000 + (a.master > 0 and 1 or 0) * 100000 + unitDataA.stars * 10000 + a.evolutionCount * 1000 + a.level
		local factorB = b.choose * 1000000 + (b.master > 0 and 1 or 0) * 100000 + unitDataB.stars * 10000 + b.evolutionCount * 1000 + b.level
	...
	numberBg:anch(0, 1):pos(8, self.size.height - 7):addTo(self)
	...
	filterBar:anch(0, 1):pos(320, self.size.height - 3):addTo(self, 100)
	...
	closeBtn:anch(0, 0):pos(self.size.width, self.size.height):addTo(self)

end

function WakeHeroChooseLayer:listHeros()
...
function WakeHeroChooseLayer:getLayer()
	return self.mask:getLayer()
end

return WakeHeroChooseLayer